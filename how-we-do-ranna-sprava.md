...
- Public archive issue URLs must use the date-based path format `/archiv/DD/MM/YYYY/`. Numeric issue pages in `/vydania/[cislo]/` still exist as the underlying issue files, but home and archive listings should open the date-based archive URL.
...
- Publish note (2026-04-14): issue `#76` was published from an already prepared local build — HTML + `sources.md`, market/weather already present, overlap check rerun, Brevo HTML ready, podcast TXT ready, and the date-based archive alias `/archiv/14/04/2026/` was included in the push. No template change.
- Publish note (2026-04-18): issue `#80` was built with the standard pipeline — HTML + `sources.md`, market snapshot, weather snapshot, overlap check, Brevo export, podcast TXT, and the date-based archive alias `/archiv/18/04/2026/`. No workflow change.
- Publish note (2026-04-20): issue `#82` was built with the standard pipeline — HTML + `sources.md`, market snapshot, weather snapshot, overlap check, Brevo export, podcast TXT, and the date-based archive alias `/archiv/20/04/2026/`. No workflow change.
- Publish note (2026-04-21): issue `#83` was built with the standard pipeline — HTML + `sources.md`, market snapshot, weather snapshot, overlap check, Brevo export, podcast TXT, and the date-based archive alias `/archiv/21/04/2026/`. No workflow change.
- Maintenance note (2026-04-23): created `/co-je-ranna-sprava/` SEO landing page targeting keywords "newsletter Slovensko", "denný newsletter", "slovenský newsletter". Includes FAQPage JSON-LD (6 Q&A pairs for AI Overviews), WebPage + BreadcrumbList schema, full OG/Twitter meta, GA, 5-line favicon. Added to sitemap.xml (priority 0.9). Footer interlink added to index.html. GEO action plan item #6 complete.
- Maintenance note (2026-04-24): patched `generate-sitemap.ps1` to emit all evergreen static pages (`co-je-ranna-sprava`, `o-nas`, `archiv`, 6× `temy/*`) with file-mtime `<lastmod>`. Added content-body anchor link from `index.html` hero-sub ("Ranná Správa") → `/co-je-ranna-sprava/` with keyword-rich surrounding copy ("slovenský denný newsletter") for in-body interlink signal (nav + footer links already existed). Sitemap grew 81 → 92 URLs. Marketing plan SEO tactic #6 ("/co-je-ranna-sprava SEO page + interlink from homepage") fully wired: page, schema, nav/footer/hero-body links, sitemap, robots.txt sitemap directive. No per-issue workflow change.
- Maintenance note (2026-04-23): backfilled NewsArticle JSON-LD + canonical + hreflang + meta description + OG meta + og:image + Twitter cards to all 34 issue pages (#48–#81) via new `generate-issue-schema.ps1` script. Issues #82–#85 already had schema. #492 skipped (duplicate anomaly). Script added to repo root — run for each new issue at publish time. GEO-AUDIT-REPORT.md Week 2 item updated to reflect full backfill.
- Maintenance note (2026-04-23): implemented IndexNow protocol for Bing/Copilot indexing. Key `7c1b25348bb644d383257abbe2001986` stored in repo root key file. New script `ping-indexnow.ps1` pings `api.indexnow.org` after each issue push. Added to AI publish workflow. Initial bulk ping: all 41 sitemap URLs accepted (202). Replaced `og:image` SVG with PNG (1200x630, generated via `generate-og-image.ps1`) on all 9 static HTML pages; `generate-topic-pages.ps1` template updated to use `.png` going forward. GEO audit quick wins: IndexNow (+15 pts Copilot), og:image PNG fix applied.
- Maintenance note (2026-04-23): generated full favicon set — `favicon.ico` (32×32), `favicon-192.png`, `favicon-512.png`, `apple-touch-icon.png` (180×180) — via System.Drawing PowerShell script, alongside existing `favicon.svg`. Updated `<link>` block in all 9 HTML files (`index.html`, `o-nas/index.html`, `vydania/82/index.html`, 6× `temy/*/index.html`) and in `design-and-structure-spec.md` mandatory head template. Reason: Google SERP showed fallback globe icon because SVG-only favicon with 32×32 viewBox and no raster/apple-touch fallbacks is under-indexed. Post-deploy: submit URL inspection + re-index request in Search Console.
...
- After updating `issues.js`, the AI must also run `generate-archive-date-pages.ps1` so every issue has a public static alias page at `/archiv/DD/MM/YYYY/`.
...
- After `git push`, the AI must notify Bing/Copilot via IndexNow by running:
  `powershell -ExecutionPolicy Bypass -File .\ping-indexnow.ps1 -Issue [cislo]`
  This pings `api.indexnow.org` with the new issue URL (202 = accepted). Key file `7c1b25348bb644d383257abbe2001986.txt` lives at repo root. The AI must never ask the user to run this manually.
...
- Every issue `vydania/[cislo]/index.html` **must** include `<meta name="description">` right after the viewport meta. Value = the `preview` field from `issues.js` (same text, max 160 chars):
  ...
  ```
- Every issue **must** include a "Súvisiace vydania" block just before `<div class="foot">` — 3 links to thematically related past issues. Pick based on shared topic with the Hlavná téma. Style uses `.sec-label` + `.related` / `.related-item` classes (defined in the issue CSS). This improves internal linking for crawlers.
...
- Every issue `vydania/[cislo]/index.html` **must** include a `NewsArticle` JSON-LD block with `"author": { "@id": "https://rannasprava.sk/o-nas/#adam-hodosi" }` and a matching `Person` node (`name: "Adam Hodoši"`, `jobTitle: "Zakladateľ a šéfredaktor"`, `url: "https://rannasprava.sk/o-nas/"`) in the same `@graph`. inside `<head>` (just before `</head>`). This is mandatory for SEO/GEO — without it AI crawlers cannot identify the page as a news article. Fill `[cislo]`, `[YYYY-MM-DD]`, and `[hlavný nadpis príbehu]` (use the `story-hed` of the Hlavná téma section):
  ```html
...
After adding a new issue to `issues.js`, or after any change that affects archive routing/helpers inside `issues.js`, the AI must update the `?v=` cache-busting query string on the `issues.js` script tag in **both** of these files:
...
## Archive date aliases — mandatory

Every issue must be publicly reachable through a date-based archive URL in this exact format:

- `/archiv/DD/MM/YYYY/`

Examples:

- issue `#71` from `2026-04-09` => `/archiv/09/04/2026/`
- issue `#75` from `2026-04-13` => `/archiv/13/04/2026/`

Implementation rules:

- The static alias pages are generated by `generate-archive-date-pages.ps1`.
- These archive date URLs may redirect to the underlying issue HTML in `/vydania/[cislo]/`.
- Home page issue cards and the archive listing must open the date-based archive URL, not the numeric `/vydania/[cislo]/` path.
- The root archive landing page remains `https://rannasprava.sk/archiv/`.
- If more than one issue shares the same date, the date URL becomes a same-day archive page and can target the exact issue via hash, for example `/archiv/17/03/2026/#issue-50`.
